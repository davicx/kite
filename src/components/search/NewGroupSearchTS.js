...
async function friendSearchAPI(currentUser,searchString, newGroupUsers) {   
    if (searchString.length > 0) {
        ...
        for (let i = 0; i < allFriends.length; i++) {
            ...
            for (let j = 0; j < newGroupUsers.length; j++) {
                console.log(newGroupUsers[j].friendName + " " + allFriends[i].friendName)
                if(newGroupUsers[j].friendName.toLowerCase() ==  allFriends[i].friendName.toLowerCase()) {
                    //availableFriends.add(availableFriends.delete())
                    addUser = false
                }                        
            }   

            if(addUser == true) {
                availableFriends.push(allFriends[i]);
            }
            console.log("     ")
        } 
...
function NewGroupSearch({ api, currentUser, newGroupUsers }) {
